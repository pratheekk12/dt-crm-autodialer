...
export default function CreateTicket(props, dealerDetails) {
  ...
  useEffect(() => {
    ...
    async function getItems() {
      ...
      if (!unmounted) {
        ...
        setTicketTypes(body.data.filter(val => val.active === true).map(({ _id, ticketType }) => ({
          label: ticketType,
          value: _id
        }))
        );
  ...
  useEffect(() => {
    ...
    async function getItems() {
      ...
      if (!unmounted) {
        setMedium(
          body.data.filter(val => val.active === true).map(({ _id, media, nameLabel, idLabel }) => ({
            label: media,
            value: _id,
            nameLabel: nameLabel,
            idLabel: idLabel
  ...
  useEffect(() => {
    ...
    async function getItems() {
      ...
      if (!unmounted) {
        ...
        setCategories(
          body.data.filter(val => val.active === true).map(({ _id, category }) => ({
            label: category,
            value: _id
          }))
        );
  ...
  const getSubCategories = cat => {
    ...
    async function getItems() {
      ...
      if (!unmounted) {
        setSubCategories(
          body.data.filter(val => val.active === true).map(({ _id, subCategory }) => ({
            label: subCategory,
            value: _id
          }))
        );
  ...
  const getSubCategoryItems = (cat, sct) => {
    ...
    async function getItems() {
      ...
      if (!unmounted && body.data) {
        setSubCategoryItems(
          body.data.filter(val => val.active === true).map(({ _id, subCategoryItem }) => ({
  ...
  useEffect(() => {
    ...
    async function getItems() {
      ...
      if (!unmounted) {
        setPriorities(
          body.data.filter(val => val.active === true).map(({ _id, priority, sla }) => ({
            label: priority,
            value: _id,
            sla
          }))
  ...
  useEffect(() => {
    ...
    async function getItems() {
      ...
      if (!unmounted) {
        setStatuses(
          body.data.filter(val => val.active === true).map(({ _id, status, slahold, closed, color }) => ({
            label: status,
            value: _id,
            slaOnHold: slahold,
            closed: closed,
  ...
  const getDepartments = () => {
    ...
    async function getItems() {
      ...
      if (!unmounted) {
        ...
        setDepartments(
          body.data.filter(val => val.active === true).map(({ _id, department }) => ({
            label: department,
            value: _id
          }))
        );
  ...
  useEffect(() => {
    ...
    async function getItems() {
      ...
      if (!unmounted) {
        ...
        setTeams(
          body.data.filter(val => val.active === true).map(({ _id, team }) => ({
            label: team,
            value: _id
          }))
        );
  ...
  useEffect(() => {
    ...
    async function getItems() {
      ...
      if (!unmounted) {
        if (
          ...
          body.data[0]
            ? setExecutive({
              label: '-- Select --',
              value: '-- Select --',
              executiveEmail: '-- Select --',
              executiveMobile: '-- Select --'
            })
            : setExecutive({});
          props.setClick(createTicket);
        }
        ...
        setExecutives(
          body.data.filter(val => val.active === true).map(
  ...
  const createTicket = () => {
    if (!distributorId || distributorId.toString().replace(' ', '').length === 0 || !distributorName || distributorName.replace(' ', '').length === 0 || !distributorMobile || distributorMobile.replace(' ', '').length === 0 || !distributorEmail || distributorEmail.replace(' ', '').length === 0 || !category || category.label === '-- Select --' || (subCategories.length > 0 && !subCategory) || (subCategories.length > 0 && subCategory.label === '-- Select --') || (subCategoryItems.length > 0 && !subCategoryItem) || (subCategoryItems.length > 0 && subCategoryItem.label === '-- Select --') || !ticketType || ticketType.label === '-- Select --' || !priority || priority.label === '-- Select --' || !department || department.label === '-- Select --' || !team || team.label === '-- Select --' || !executive || executive.label === '-- Select --' || !ticketSubject || ticketSubject.replace(' ', '').length === 0 || !ticketDescription || ticketDescription.replace(' ', '').length === 0 || !remarks || remarks.replace(' ', '').length === 0) {
      alert("Please fill fields marked in Red!");
      props.setOpen(true);
    }
    else {
      var result = addRow();
  ...
  const addRow = async () => {
    ...
    var apiParam = {
      ...
      headers: {
        ...
        createdByName,
        createdById,
        updatedByName: userData.username,
        updatedById: userData.email,
        ticketSubject,
        ticketDescription,
        remarks,
        ...
        <div style={{ width: '100%' }}>
          ...
          {props.ticket_id ? (
            ''
              ...
              <Button
                className="btn btn-primary"
                onClick={getDistributorByMobile}
              >
                <SearchIcon color="primary" />
              </Button>
            )}
          <TextField
            error={(distributorEmail || "").split('@').length !== 2 || (distributorEmail || "").split('@')[1].split('.').length < 2}
            id="dn"
            size="small"
          ...
          {subCategories.length > 0 ? (
            <TextField
              id="subcategories"
              error={subCategory.value === '-- Select --'}
              select
              size="small"
          ...
          {subCategoryItems.length > 0 ? (
            <TextField
              id="subcategoryitems"
              error={subCategoryItem.value === '-- Select --'}
              select
              size="small"
          ...
          <TextField
            id="priorities"
            error={priority.value === '-- Select --'}
            select
            value={priority.value}
            disabled={subCategories.length < 1 || !subCategory || !subCategory.label || subCategory.label === '-- Select --'}
            size="small"
            label="Priority"
            InputLabelProps={{ shrink: true }}
            SelectProps={{
          ...
          <TextField
            id="sm"
            error={!team || !team.label || team.label === '-- Select --'}
            select
            size="small"
            label="Team"
            SelectProps={{
            ...
            disabled={
              (assignAccess < 2 &&
                !(assignAccess === 1 && roleAccess.teamId === team.value)) || status.slaOnHold
            }
            onChange={e => {
              setTeam({
                value: e.target.value,
          ...
          <TextField
            error={!executive || !executive.label || executive.label === '-- Select --'}
            id="sm"
            select
            size="small"
            label="Executive"
